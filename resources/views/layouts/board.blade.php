...
<div class="background"><img src="https://cdn.imageupload.workers.dev/vtupnEWP_tlo.png">
    ...
    @else <div class="pos8null" ><div><a href="newBuild/wall"><img src="https://cdn.imageupload.workers.dev/GUkKYoRS_flaga_wycieta.png"></a></div></div>
    @endif
    @for($i=1;$i<=7;$i++)
        @if(isset($sends[$i]['link']))
            <div class="pos{{$i}}"><a href="{{$sends[$i]['name']}}"><img src="{{$sends[$i]['link']}}"><ul><li>{{$sends[$i]['name']}}</li></ul></a></div>
            @else <div class="pos{{$i}}" ><a href="newBuild/{{$i}}"><img src="https://cdn.imageupload.workers.dev/GUkKYoRS_flaga_wycieta.png"></a></div>
        @endif
    @endfor
